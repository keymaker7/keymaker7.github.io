...
![터미널 멀티플렉서 비교 카드뉴스](attachments/2026-05-11_tmux-cmux-덕테이프_카드뉴스.png)
...
```
[본진 Mac mini]
  ├─ tmux: SSH로 원격 서버 관리·로그 모니터링·서버 재기동
  ├─ cmux: Claude Code 여러 에이전트 (가디언즈·트랜스포머 등) 병렬 진행
  └─ 덕테이프 정신: 한 도구가 안 되면 또 다른 기본 도구로
```

- **새벽 서버 장애** → tmux로 SSH + 로그 추적 + 핫픽스
- **블로그·콘텐츠 자동화** → cmux + Claude Code로 페르소나 9봇 동시 운영
